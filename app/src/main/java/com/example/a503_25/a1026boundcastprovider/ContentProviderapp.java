...
public class ContentProviderapp extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_content_providerapp);
        ...
        //os가 6.0 이상일 때 동적인 권한 요청 - 연락처, 외부 저장 장치 권한이 없으면 주기
    ...
    public void onClick(View v){
       ...
       // if(v == contactBtn){
            ...
            //액티비티가 화면에 출력하고 출력된 액티비티가 종료되면 10이라는 값과 onActivityResult라는 메소드를 호출합니다.
            startActivityForResult(intent,10);

        }
        //갤러리 앱 실행하기
        else if(v.getId() == R.id.lab2_gallery){
            //Log.e("갤러리앱","갤러리앱 실행");
            //갤러리 앱 호출
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        //resultCode == RESULT_OK저쪽에서 확인 누른거
        //연락처 앱에서 확인을 눌렀다면
       if(requestCode == 10 && resultCode == RESULT_OK){
            //선택한 데이터 가져오기
           String id = Uri.parse(data.getDataString()).getLastPathSegment();
           Cursor cursor = getContentResolver().query(ContactsContract.Data.CONTENT_URI,new String[]{ContactsContract.Contacts.DISPLAY_NAME, ContactsContract.CommonDataKinds.Phone.NUMBER},ContactsContract.Contacts. _ID+"=" + id,null,null);
            //첫번째 행으로 이동
           cursor.moveToNext();
    ...
    private void insertImageView(String filepath){
        ...
        if(filepath.equals("") == false){
            ...
            Bitmap bitmap =
                    BitmapFactory.decodeFile(filepath, imgOptions);
    ...
    private String getFilePathFromDocumentUrl(
        ...
        String type = split[0];
        Uri contentUri = null;
        //NullPointerExceptiond을 방지하기 위해서
        //상수를 가지고 비교
